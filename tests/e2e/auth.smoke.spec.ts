...
test("access request reports success without a client-side error", async ({
  ...
  await page.getByLabel("Email").fill("unknown-browser-user@example.com");
  await page
    .getByRole("button", {
      name: "Continue to the authenticated destination.",
    })
    .click();
  ...
  await expect(page.getByText("Service error")).not.toBeVisible();
});
